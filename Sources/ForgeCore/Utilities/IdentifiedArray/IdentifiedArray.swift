...
public struct IdentifiedArray<ID: Hashable, Element> {
    ...
    @inlinable
    @inline(__always)
    public func find(element: Element) -> Element? {
        self._dictionary[self._id(element)]
    }
...
extension IdentifiedArray where Element: Identifiable, Element.ID == ID {
    @inlinable
    @discardableResult
    public mutating func replace(element: Element, with newElement: Element) -> Bool {
        guard let elementIndex = index(id: element.id) else {
            return false
        }

        self[elementIndex] = newElement
        return true
    }
}
